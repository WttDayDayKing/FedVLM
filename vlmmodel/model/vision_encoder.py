...
def build_vision_tower(model_cfg, **kwargs):
    ...
    if is_absolute_path_exists or vision_tower.startswith("openai") or vision_tower.startswith("laion"):
        ...
        if vision_tower_type == "clip":
            # return open_clip.create_model_and_transforms('ViT-B-32', pretrained='laion2b_s34b_b79k')
            return CLIPVisionTower(vision_tower, args=model_cfg, **kwargs)
    raise ValueError(f'Unknown vision tower: {vision_tower}')

# def build_vision_tower2(model_cfg, **kwargs):
#     model, _, preprocess = open_clip.create_model_and_transforms('hf-hub:openai/clip-vit-large-patch14-336')
#     return model
