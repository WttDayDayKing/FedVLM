...
def get_fed_showo_sft_trainer(config, model, tokenizer, training_args, local_dataset, eval_dataset, compute_metrics, data_collator, global_dict, local_auxiliary, global_auxiliary,preprocess_logits_for_metrics,resume_from_checkpoint=None):
    fed_args=config.fed
...
def get_local_showo_sft_trainer(config, model, tokenizer, training_args, local_dataset, eval_dataset, compute_metrics, data_collator,preprocess_logits_for_metrics,resume_from_checkpoint=None):
    fed_args=config.fed
...
class VLMTrainer(Trainer):
    def __init__(self, model, tokenizer,training_args,config,train_dataset, eval_dataset,compute_metrics,data_collator,preprocess_logits_for_metrics,resume_from_checkpoint=None):
    ...
    def create_optimizer(self):
        ...
        optimizer_config = self.config.optimizer.params
    ...
    def prediction_step(
        ...
        # print(type(outputs))
        # print(outputs.keys() if isinstance(outputs, dict) else "not dict")
        labels= outputs["labels"]
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        for name, param in model.named_parameters():
            ...
            if not param.requires_grad:
                ...
                if param.is_cuda:
                    loss += self.mu / 2 * torch.norm(param - self.global_state[name].cuda().detach()) ** 2
                else:
                    # loss += self.mu / 2 * torch.norm(param - self.global_state[name].detach()) ** 2
                    loss += self.mu / 2 * torch.norm(param.cpu() - self.global_state[name].detach()) ** 2
                    loss.cuda()
